...
{
    p = BA4 + ((j - l2) * (u3 - l3 + 1) * (u1 - l1 + 1) + (k - l3) * (u1 - l1 + 1) + (i - l1));
    *p = x;
}
...
//---------------------------------------------------------------------
// ฟังชั่นหา Plane, Row, Column จาก offset (3D Array: Row-Column-Plane)
void GetIndices3D_RCP(long offset, int *pi, int *pj, int *pk)
{
    int plane_size = (u1 - l1 + 1); // 3
    int col_size = (u3 - l3 + 1);   // 5

    int row = offset / (col_size * plane_size);
    long remainder1 = offset % (col_size * plane_size);

    int col = remainder1 / plane_size;
    int plane = remainder1 % plane_size;
...
int main()
{
    ...
    printf("\n(Row-Column-Plane)"); // แสดงข้อความว่าฟั่งชั่นทำงานแบบไหน
    i = 2;
    j = 2;
    k = 2;
    number = 99;         // set value เข้าฟั่งชั่น
    ...
    orig_j = j;
    orig_k = k;
    GetIndices3D_RCP(offset_A4, &i, &j, &k);
